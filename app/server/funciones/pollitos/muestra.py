...
async def listar_control(control_data: dict) -> dict:
            ...
            async for notificacion in control_collection.find(query,{"_id":0,"id_control":1,"condicion_control":1,"cantidad_control":1,"estado_control":1,"created_at":1}).sort({"created_at":-1}):
...
async def eliminar_control(control_data: dict) -> dict:
    if control_data['especifico']:
        ...
        especifico = await control_collection.find_one({"id_control":control_data['especifico'],"estado_control":1},{"_id":0 ,"condicion_control":1})             
        print(especifico)
        if especifico :
            ...
            res = "FAIL"
        #Guardar en Log 
        log =procesar_log("Se elimino Control :  ",control_data['id_usuario'],control_data['especifico'])
        guardar_log = await log_general_collection.insert_one(log)
        return res
    else :
        return "SIN_ESPECIFICO"
...
async def reestablecer_control(control_data: dict) -> dict:
    if control_data['especifico']:
        ...
        especifico = await control_collection.find_one({"id_control":control_data['especifico'],"estado_control":0},{"_id":0 ,"condicion_control":1})             
        if especifico :
            ...
            res = "FAIL"
        #Guardar en Log 
        log =procesar_log("Se reestablece el Control :  ",control_data['id_usuario'],control_data['especifico'])
        guardar_log = await log_general_collection.insert_one(log)
        return res
    else :
        return "SIN_ESPECIFICO"
