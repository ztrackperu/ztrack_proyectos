...
async def guardar_usuario(pre_proyecto_data: dict) -> dict:
    ...
    if validar_token and pre_proyecto_data['user_c']:
        if validar_token['fecha_fin']>fecha_actual :            
            coincidencia_dato = await pre_proyecto_collection.find_one({"nombre_pre_proyecto ":pre_proyecto_data['nombre_pre_proyecto '] ,"estado_usuario":1},{"_id":0})
            proyecto_ok ="FAIL"
            ...
            if id_value==0 :
                ...
                if coincidencia_dato :
                    ...
                    proyecto_ok = await pre_proyecto_collection.find_one({"_id": guardar_usuario.inserted_id},{"_id":0,"id_pre_proyecto":1,"nombre_pre_proyecto":1})
                    ...
                    #Guardar en Log 
                    log =procesar_log("PRE PROYECTO GUARDADO",pre_proyecto_data['user_c'],pre_proyecto_data['user_proyecto'])
                ...
                guardar_pre_proyecto_historico = await h_pre_proyecto_collection.insert_one(pre_proyecto_historico)
                #Guardar en Log 
                log =procesar_log("PRE PROYECTO EDITADO por ",pre_proyecto_data['user_c'],pre_proyecto_data['id_usuario'])
                guardar_log = await log_general_collection.insert_one(log)
    ...
    if validar_token :
        if validar_token['fecha_fin']>fecha_actual :
            if pre_proyecto_data['especifico']:
                ...
                especifico = await pre_proyecto_collection.find_one({"id_usuario":pre_proyecto_data['especifico'],"estado_usuario":1},{"_id":0 ,"clave_proyecto":0,"token_proyecto":0})             
                #Guardar en Log 
                log =procesar_log("Se solicito info de usuario :  ",pre_proyecto_data['id_usuario'],pre_proyecto_data['especifico'])
                guardar_log = await log_general_collection.insert_one(log)
                #actualizar vida de token 
    ...
    if validar_token :
        if validar_token['fecha_fin']>fecha_actual :
            ...
            if pre_proyecto_data['tipo_usuario']==1 :
                ...
                #query = {"estado_usuario":1,"user_c":pre_proyecto_data['id_usuario']}
            else :
                query = {"created_at": {"$gte": fecha_inicio, "$lte": fecha_fin,"estado_usuario":1,"user_c":pre_proyecto_data['id_usuario']}}
            print("*****ACUMULADO********")
            print(query)
            print("*****ACUMULADO********")

            async for notificacion in pre_proyecto_collection.find(query,{"_id":0,"id_usuario":1,"user_proyecto":1,"nombres_usuario":1,"estado_usuario":1,"tipo_usuario":1,"created_at":1,"apellidos_usuario":1}).sort({"created_at":-1}):
                notificacions.append(notificacion)
            res = {"fecha_inicio" :fecha_inicio,"fecha_fin" :fecha_fin ,"resultado" :notificacions}
            #guardar en log
            log =procesar_log("LISTADO DE USUARIOS POR ",pre_proyecto_data['id_usuario'],pre_proyecto_data['tipo_usuario'])
            guardar_log = await log_general_collection.insert_one(log)
            return res 
        else :
            #cancelar el token 
    ...
    if validar_token :
        if validar_token['fecha_fin']>fecha_actual :
            if pre_proyecto_data['especifico']:
                ...
                objeto = {"estado_usuario":0,"user_m":pre_proyecto_data['id_usuario'],"updated_at":datetime.now()}   
                especifico = await pre_proyecto_collection.find_one({"id_usuario":pre_proyecto_data['especifico'],"estado_usuario":1},{"_id":0 ,"user_proyecto":1})             
                if especifico :
                    especifico_ok = await pre_proyecto_collection.update_one({"id_usuario":pre_proyecto_data['especifico'],"estado_usuario":1},{"$set":objeto})
                    res = "OK"
                else :
                    res = "FAIL"
                #Guardar en Log 
                log =procesar_log("Se elimino usuario :  ",pre_proyecto_data['id_usuario'],pre_proyecto_data['especifico'])
                guardar_log = await log_general_collection.insert_one(log)
                #actualizar vida de token 
                tiempo_extendido =validar_token['fecha_fin'] + timedelta(minutes=30) if pre_proyecto_data['id_usuario']==1 else datetime.now() + timedelta(minutes=30)
                extender_token = await token_proyecto_collection.update_one({"token_proyecto":pre_proyecto_data['token_proyecto'],"estado_token":1},{"$set":{"fecha_fin":tiempo_extendido}}) 
...
async def reestablecer_usuarios(pre_proyecto_data: dict) -> dict:
    ...
    if validar_token :
        if validar_token['fecha_fin']>fecha_actual :
            if pre_proyecto_data['especifico']:
                #realizar secuencia para cambiar estado 0 
                objeto = {"estado_usuario":1,"user_m":pre_proyecto_data['id_usuario'],"updated_at":datetime.now()}   
                especifico = await pre_proyecto_collection.find_one({"id_usuario":pre_proyecto_data['especifico'],"estado_usuario":0},{"_id":0 ,"user_proyecto":1})             
                if especifico :
                    especifico_ok = await pre_proyecto_collection.update_one({"id_usuario":pre_proyecto_data['especifico'],"estado_usuario":1},{"$set":objeto})
                    res = "OK"
                else :
                    res = "FAIL"
                #Guardar en Log 
                log =procesar_log("Se reestablece el usuario :  ",pre_proyecto_data['id_usuario'],pre_proyecto_data['especifico'])
                guardar_log = await log_general_collection.insert_one(log)
                #actualizar vida de token 
                tiempo_extendido =validar_token['fecha_fin'] + timedelta(minutes=30) if pre_proyecto_data['id_usuario']==1 else datetime.now() + timedelta(minutes=30)
                extender_token = await token_proyecto_collection.update_one({"token_proyecto":pre_proyecto_data['token_proyecto'],"estado_token":1},{"$set":{"fecha_fin":tiempo_extendido}}) 
